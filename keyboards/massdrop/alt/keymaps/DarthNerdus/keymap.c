...
enum alt_keycodes {
    ...
    WOWTYPE,               //Toggle subset typing keyset and turn off keymaps in WoW MB
};
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [_BASE] = LAYOUT_65_ansi_blocker(
        TD(ESC), KC_1,    KC_2,    KC_3,    KC_4,    KC_5,    KC_6,    KC_7,    KC_8,    KC_9,    KC_0,    KC_MINS, KC_EQL,  KC_BSPC, TD(SES),  \
    ...
    [_WOWMB] = LAYOUT_65_ansi_blocker(
        _______, KC_F1,   KC_F2,   KC_F3,   KC_F4,   KC_F5,   KC_F6,   XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, _______, MB_BRD,  \
        _______, _______, _______, _______, KC_F,    KC_T,    _______, _______, _______, _______, _______, _______, _______, WOWTYPE, MB_KEY,  \
    ...
    [_WOWTYPE] = LAYOUT_65_ansi_blocker(
        TD(ESC), KC_F1,   KC_F2,   KC_F3,   KC_F4,   KC_F5,   KC_F6,   KC_F7,   KC_F8,   KC_F9,   KC_F10,  KC_F11,  KC_F12,  KC_BSPC, TD(SES),  \
        KC_TAB,  KC_Q,    KC_W,    KC_E,    KC_R,    KC_T,    KC_Y,    KC_U,    KC_I,    KC_O,    KC_P,    KC_LBRC, KC_RBRC, WOWTYPE, TD(HME), \
        KC_CAPS, KC_A,    KC_S,    KC_D,    KC_F,    KC_G,    KC_H,    KC_J,    KC_K,    KC_L,    KC_SCLN, KC_QUOT,          KC_ENT,  KC_VOLU, \
        KC_LSFT, KC_Z,    KC_X,    KC_C,    KC_V,    KC_B,    KC_N,    KC_M,    KC_COMM, KC_DOT,  KC_SLSH, KC_RSFT,          KC_UP,   TD(MUT), \
        KC_LCTL, KC_LGUI, KC_LALT,                            KC_SPC,                             TD(ALT), TD(FN),  KC_LEFT, KC_DOWN, KC_RGHT  \
    ),    
    [_FUNCTIONS] = LAYOUT_65_ansi_blocker(
        ...
        _______, _______, _______,                            XXXXXXX,                            KC_RALT, XXXXXXX, KC_HOME, KC_PGDN, KC_END   \
    )
};
...
const uint8_t PROGMEM ledmap[][DRIVER_LED_TOTAL][3] = {
    ...
    [_WOWMB] = {
        ...
        RED,     RED,     BRED,    RED,     GREEN,   RED,     RED,     RED,     RED,     RED,     RED,     RED,     RED,     GOLD,    GOLD,    \
    ...
    [_WOWTYPE] = {
        RED,     RED,     RED,     RED,     RED,     RED,     RED,     RED,     RED,     RED,     RED,     RED,     RED,     RED,     RED,     \
        RED,     JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, GOLD,    RED,     \
        RED,     JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR,          JR_AZUR, RED,     \
        RED,     JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR, JR_AZUR,          JR_AZUR, RED,     \
        RED,     RED,     RED,                                RED,                                RED,     RED,     JR_AZUR, JR_AZUR, JR_AZUR, \
        _______\
    },
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case WOWTYPE:
            if (record->event.pressed) {
                if (get_highest_layer(layer_state) == _WOWMB) {
                    layer_on(_WOWTYPE);
                    register_code(KC_LSHIFT);
                    register_code(KC_LALT);
                    register_code(KC_M);
                } else if (get_highest_layer(layer_state) == _WOWTYPE) {
                    layer_off(_WOWTYPE);
                    register_code(KC_LSHIFT);
                    register_code(KC_LALT);
                    register_code(KC_M);
